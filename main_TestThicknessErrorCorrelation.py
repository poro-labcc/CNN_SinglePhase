    ...
    with torch.no_grad():
        for batch_idx, (batch_inputs, batch_targets) in enumerate(dataloader):
            ...
            for samp in range(current_bs):
                ...
                min_radius          = 5
                max_radius          = 17
                inlets              = np.zeros_like(void_mask, dtype=bool)
                inlets[0, :, :]     = 1  
                outlets             = np.zeros_like(void_mask, dtype=bool)
                outlets[-1, :, :]   = 1 
                im_transp = ps.filters.trim_nonpercolating_paths(void_mask, inlets=inlets, outlets=outlets)
                ...
                dissimilarity_perc  = 100 - real_percentage
...
for idx, (dataname, res_list) in enumerate(all_results.items()):
    ...
    # Scatter points
    ax.scatter(x_dissim, y_r_coeffs, alpha=0.3, s=60, color=c, edgecolors='black', 
